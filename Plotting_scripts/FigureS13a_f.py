...
Script for plotting Figure S13a-f
...
regridder = xe.Regridder(VAR, ds_out, 'bilinear',periodic=True)
VAR_gr = regridder(VAR)
diat=np.roll(VAR_gr,-210)

#%% Select NA boundaries (45N - 70N x 270E - 30E)
diat_W=diat[:,:,135:160,240]
diat_E=diat[:,:,135:160,-30]

diat_N=diat[:,:,160,240:-30]
diat_S=diat[:,:,135,240:-30]
...
flux_E=np.nansum(diat_E*uO_E,axis=1)*10
flux_W=np.nansum(diat_W*uO_W,axis=1)*10
flux_N=np.nansum(diat_N*vO_N,axis=1)*10
flux_S=np.nansum(diat_S*vO_S,axis=1)*10

#%%
flux_E1=np.nansum(np.mean(diat_E,axis=0)*uO_E,axis=1)*10
flux_W1=np.nansum(np.mean(diat_W,axis=0)*uO_W,axis=1)*10
flux_N1=np.nansum(np.mean(diat_N,axis=0)*vO_N,axis=1)*10
flux_S1=np.nansum(np.mean(diat_S,axis=0)*vO_S,axis=1)*10

flux_E2=np.nansum(diat_E*np.mean(uO_E,axis=0),axis=1)*10
flux_W2=np.nansum(diat_W*np.mean(uO_W,axis=0),axis=1)*10
flux_N2=np.nansum(diat_N*np.mean(vO_N,axis=0),axis=1)*10
flux_S2=np.nansum(diat_S*np.mean(vO_S,axis=0),axis=1)*10
...
plt.tight_layout()
plt.legend(['$\overrightarrow{V}$ P$_{di}$','$\overrightarrow{V}$ $\langle$P$_{di}$⟩','$\langle\overrightarrow{V}$⟩ P$_{di}$'],fontsize=FS-3,ncol=3)
plt.savefig('figureS13_1.png', format='png', dpi=300)
...
plt.tight_layout()
#plt.legend(['$\overrightarrow{V}$ P$_{di}$','$\overrightarrow{V}$ $\langle$P$_{di}$⟩','$\langle\overrightarrow{V}$⟩ P$_{di}$'],fontsize=FS-3,ncol=3)
plt.savefig('figureS13_2.png', format='png', dpi=300)
...
plt.yticks(fontsize=FS-3)
plt.tight_layout()
plt.savefig('figureS13_3.png', format='png', dpi=300)
...
plt.yticks(fontsize=FS-3)
plt.tight_layout()
plt.savefig('figureS13_4.png', format='png', dpi=300)
...
plt.yticks(fontsize=FS-3)
plt.tight_layout()
plt.savefig('figureS13_5.png', format='png', dpi=300)

#%% Integrate over depth and select NA (45N - 70N x 270E - 30E)
Diat=np.nansum(diat[:,:,135:160,240:-30],axis=1)*10
...
VAR_di=np.zeros((np.shape(Diat)))

for i in range(np.size(Diat[:,0,0])):
    VAR_di[i,:,:]=Diat[i,:,:]*Area
    
VAR_DI=np.nansum(np.nansum(VAR_di,axis=1),axis=1)

#%%
tm=5
Var_tot = pd.DataFrame(VAR_DI)
rolling_windows = Var_tot.rolling(tm)
...
plt.xlabel('Time',fontsize=FS)
plt.ylabel('Biomass (di) [10$^{10}$ mol C]',fontsize=FS)
plt.grid()
plt.xlim([2020,2100])
plt.xticks(a,fontsize=FS-3)
plt.yticks(fontsize=FS-3)
plt.tight_layout()
plt.savefig('figureS13_6di.png', format='png', dpi=300)